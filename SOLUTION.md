...
As the teams repeated this cycle, the prototypes started to integrate together: the React app fetched and sent user data to the backend, and the backend stored the results of the recommendation model. Because the work between teams became more fluid, we had each team demo its work for each new feature/update and gathered user feedback from other teams. As reflected in our testing, this process ensured all project developers were kept up-to-date and could effectively contribute to the same team vision and helped us rapidly connect all the components into a full website during the past few weeks.
